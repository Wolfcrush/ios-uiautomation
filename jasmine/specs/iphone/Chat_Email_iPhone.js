describe("Banjo - Chat. Send Email.", function() {
...
//---------------------------------------
it("CHAT.SEND.TEXT", function() {
	UIALogger.logStart("CHAT.SEND.TEXT");

	Win.textViews()[0].tap();
	app.keyboard().typeString("test@teambanjo.com");
	Screenshot(num++);
	Win.buttons()[2].tap();
	target.delay(3);
	Screenshot(num++);
	...
	if (Win.tableViews()[0].cells()[0].secureTextFields()[0].checkIsValid()) {
		UIALogger.logPass("CHAT.SEND.TEXT PASSED");
	}
	else {
		UIALogger.logFail("CHAT.SEND.TEXT FAILED");
		throw new Error("CHAT.SEND.TEXT FAILED");
	}
});
//---------------------------------------

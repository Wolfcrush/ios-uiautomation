describe("Banjo - Chat. Send a Photo (Landscape).", function() {
...
it("COMPOSE.SEARCH", function() {
	...
	Wait(Win.popover().tableViews()[0].textFields()[0], 3);
	Screenshot(num++);
	
	
	Win.popover().tableViews()[0].textFields()[0].tap();
	app.keyboard().typeString(user2_search);
	Wait(Win.popover().tableViews()[0].buttons()[1]);
	Screenshot(num++);

	if (Win.popover().tableViews()[0].cells()[user2_name].checkIsValid()) {
...
//---------------------------------------
it("CHAT.SEND.PHOTO", function() {
	
	UIALogger.logStart("CHAT.SEND.PHOTO");
	...
	if (app.actionSheet().buttons()[1].checkIsValid()) {
		...
		if (Win.tableViews()[0].cells()[0].buttons()[0].checkIsValid()) {
			UIALogger.logPass("CHAT.SEND.PHOTO PASSED");
		}
		else {
			UIALogger.logFail("CHAT.SEND.PHOTO FAILED");
			throw new Error("CHAT.SEND.PHOTO FAILED");
		}
});

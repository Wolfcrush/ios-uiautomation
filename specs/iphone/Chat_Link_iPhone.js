describe("Banjo - Chat. Send a Link.", function() {
...
//---------------------------------------
it("CHAT.SEND.LINK", function() {
	UIALogger.logStart("CHAT.SEND.LINK");

	Win.textViews()[0].tap();
	app.keyboard().typeString("http://google.com");
	Screenshot(num++);
	Win.buttons()[2].tap();
	target.delay(3);
	Screenshot(num++);

	Win.tableViews()[0].cells()[0].secureTextFields()[0].tapWithOptions({tapOffset:{x:0.5, y:0.5}, duration:0.3});
	Screenshot(num++, 3);

	if (app.toolbar().buttons()["webview back button"].checkIsValid()
		&& app.toolbar().buttons()["webview forward button"].checkIsValid()) {
		UIALogger.logPass("CHAT.SEND.LINK PASSED");
	}
	else {
		UIALogger.logFail("CHAT.SEND.LINK FAILED");
		throw new Error("CHAT.SEND.LINK FAILED");
	}

	app.toolbar().buttons()[3].tap();
	Screenshot(num++);
